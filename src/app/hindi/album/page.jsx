...
const AlbumPage = () => {
    ...
    const fetchAlbumDetails = async () => {
        setLoading(true);
        try {
            const response = await axios.get(`/api/hindiAlbum?link=${encodeURIComponent(albumLink)}`);
    ...
    return (
        ...
        <div className={styles.songsList}>
            ...
            <ul>
                {album.allDetails.map((song, index) => (
                    <li key={index} className={styles.songItem}>
                        <img src={song.albumCover} alt={song.songName} className={styles.albumCover} />
